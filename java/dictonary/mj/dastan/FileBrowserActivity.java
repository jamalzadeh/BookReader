...
Context context;
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (sel.isDirectory()) {
					...
					if (!directoryShownIsEmpty) {
						...
						String newFile=sel.getAbsolutePath();
						if(newFile.substring(newFile.lastIndexOf('.') + 1).equals("pdf")){
							...
							long pdfsize=myDbHelper.counttable("pdf");
							String isbn="";
							if(pdfsize!=0){
								isbn=myDbHelper.getimage((int) pdfsize,"pdf").getisbn();
							}
							if(!newFile.equals(isbn)){
								//lastpage=toc
								datasource.open();
								datasource.createepubbooks("pdf",newFile,newFile.substring(newFile.lastIndexOf("/")+1,newFile.length()),"","1");
								datasource.updateMatn(1, newFile.substring(1, newFile.lastIndexOf("/")),"config");
							datasource.close();
							}
							myDbHelper.close();
	...
	private void loadFileList() {
		...
		if (path.exists() && path.canRead()) {
			FilenameFilter filter = new FilenameFilter() {
				public boolean accept(File dir, String filename) {
					...
					if (currentAction == SELECT_FILE) {
						...
						if (sel.isFile() && filterFileExtension != null && sel.getName().substring(filename.lastIndexOf('.') + 1).equals("epub") ) {
							Log.v("extension",sel.getName().substring(filename.lastIndexOf('.') + 1));
							return (showReadableFile && sel.getName().endsWith(
									filterFileExtension));
						}
						if (sel.isFile() && filterFileExtension != null && sel.getName().substring(filename.lastIndexOf('.') + 1).equals("pdf") ) {
							Log.v("extension",sel.getName().substring(filename.lastIndexOf('.') + 1));
							return (showReadableFile && sel.getName().endsWith(
									filterFileExtension));
						}
						if (sel.isFile() && filterFileExtension != null && sel.getName().substring(filename.lastIndexOf('.') + 1).equals("jpub") ) {
							Log.v("extension",sel.getName().substring(filename.lastIndexOf('.') + 1));
							return (showReadableFile && sel.getName().endsWith(
									filterFileExtension));
						}
			...
			Log.v("size",Integer.toString(fList.length));
			...
			for (int i = 0; i < fList.length; i++) {
				...
				if(sel.isDirectory()) {
					Log.v("size in",Integer.toString(fList.length));
					fileList.add(fileList.size(), new Item(fList[i], drawableID, canRead));
					
				}
				if(sel.isFile()&&sel.getName().substring(sel.getName().lastIndexOf('.') + 1).equals("epub")){
					fileList.add(fileList.size(), new Item(fList[i], drawableID, canRead));
				}
				if(sel.isFile()&&sel.getName().substring(sel.getName().lastIndexOf('.') + 1).equals("pdf")){
					fileList.add(fileList.size(), new Item(fList[i], drawableID, canRead));
				}
				if(sel.isFile()&&sel.getName().substring(sel.getName().lastIndexOf('.') + 1).equals("jpub")){
					fileList.add(fileList.size(), new Item(fList[i], drawableID, canRead));
				}
